...
import { useTradingReports, useReport, useGenerateTradingReport } from "../../api/hooks";
import { usePortfolioContext } from "../../context/PortfolioContext";
import { FileText, Loader2, Sparkles, AlertTriangle, CheckCircle, Calendar, TrendingUp } from "lucide-react";
...
const TYPE_LABELS: Record<string, string> = {
  trading_daily: "DAILY",
  trading_weekly: "WEEKLY",
  trading_custom: "CUSTOM",
};

const TYPE_VARIANTS: Record<string, "info" | "positive" | "warning" | "neutral"> = {
  trading_daily: "info",
  trading_weekly: "positive",
  trading_custom: "warning",
};

export default function TradingReportsTab() {
  const { activePortfolioId } = usePortfolioContext();
  const [selectedId, setSelectedId] = useState("");
  const [customStart, setCustomStart] = useState("");
  const [customEnd, setCustomEnd] = useState("");

  const reports = useTradingReports(activePortfolioId);
  const detail = useReport(selectedId);
  const generate = useGenerateTradingReport();
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-1.5 text-xs text-muted">
          <TrendingUp size={12} />
          <span>Portfolio: <span className="font-medium text-zinc-300">{activePortfolioId}</span></span>
        </div>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => handleGenerate("trading_daily")}
          ...
          <button
            onClick={() => handleGenerate("trading_weekly")}
            disabled={generate.isPending}
          ...
          <button
            onClick={() => handleGenerate("trading_custom")}
      ...
      {generate.isPending && (
        <div className="rounded border border-accent/30 bg-accent/5 px-3 py-2.5">
          <div className="flex items-center gap-2 text-xs text-accent">
            ...
            Generating trading report… This may take 30–60 seconds.
          </div>
          <div className="mt-1.5 h-1 w-full overflow-hidden rounded-full bg-surface-overlay">
            <div className="h-full animate-pulse rounded-full bg-accent/60" style={{ width: "60%" }} />
          </div>
        </div>
      )}
      {generate.isError && (
      ...
      {generate.isSuccess && (
        <div className="flex items-center gap-2 rounded bg-positive/10 border border-positive/30 px-3 py-2 text-xs text-positive">
          <CheckCircle size={12} />
          Trading report generated successfully
        </div>
      )}

      <div className="grid gap-3 lg:grid-cols-[1fr_1.5fr]">
        {/* Report list */}
        <Panel title="Trading Reports">
          <div className="max-h-[calc(100vh-340px)] overflow-auto space-y-1">
            {list.length === 0 && (
              <p className="py-8 text-center text-xs text-muted">
                No trading reports yet — click a Generate button to create one
              </p>
            )}
            {list.map((r) => (
              <button
