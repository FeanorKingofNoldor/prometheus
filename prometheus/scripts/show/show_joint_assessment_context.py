"""Show joint Assessment context embeddings (ASSESSMENT_CTX_V0).

This script queries the `joint_embeddings` table for
`joint_type = 'ASSESSMENT_CTX_V0'` and prints a CSV summary including:
...
--------
    ...
    # List all Assessment context embeddings for a date
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        help="Filter by joint model_id (default: joint-assessment-context-v1)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    where_clauses = ["joint_type = 'ASSESSMENT_CTX_V0'", "model_id = %s"]
